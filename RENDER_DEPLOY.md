...
2. Click **Add Environment Variable** and add the following keys with the values from your local `.env` file:
   - `RPC_URL` : `https://mainnet.helius-rpc.com/?api-key=REDACTED_HELIUS_KEY`
   - `HELIUS_RPC_URL` : `https://mainnet.helius-rpc.com/?api-key=REDACTED_HELIUS_KEY`
   - `OOBE_RPC_URL` : `https://staging.oobeprotocol.ai:8080/rpc?api_key=REDACTED_OOBE_KEY`
