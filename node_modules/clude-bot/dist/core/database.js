...
function getDb() {
    if (!supabase) {
        supabase = (0, supabase_js_1.createClient)(config_1.config.supabase.url, config_1.config.supabase.serviceKey);
...
async function checkRateLimit(key, maxCount, windowMinutes) {
    const db = getDb();
...
async function markProcessed(tweetId, feature, responseTweetId) {
    const db = getDb();
    await db
        .from('processed_mentions')
        .upsert({
