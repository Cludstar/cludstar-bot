...
export class Cortex {
  ...
  constructor(private config: CortexConfig) {
    if (config.hosted) {
      ...
      // Inject Gemini config if provided
      if (config.gemini?.apiKey) {
        const { _setGeminiClient } = require('../core/claude-client');
        _setGeminiClient(config.gemini.apiKey, config.gemini.model);
      }
  ...
  async dream(opts?: DreamOptions): Promise<void> {
    ...
    if (!this.config.gemini?.apiKey) {
      throw new Error('Cortex.dream() requires gemini config');
    }
  ...
  startDreamSchedule(): void {
    ...
    if (!this.config.gemini?.apiKey) {
      throw new Error('Dream schedule requires gemini config');
    }
  ...
  async reflect(opts?: { onReflection?: (journal: any) => Promise<void> }): Promise<any> {
    ...
    if (!this.config.gemini?.apiKey) {
      throw new Error('Cortex.reflect() requires gemini config');
    }
  ...
  startReflectionSchedule(): void {
    ...
    if (!this.config.gemini?.apiKey) {
      throw new Error('Reflection schedule requires gemini config');
    }
